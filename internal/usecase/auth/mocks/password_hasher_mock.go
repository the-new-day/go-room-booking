...
// DoesMatch provides a mock function for the type MockPasswordHasher
func (_mock *MockPasswordHasher) DoesMatch(hash string, password string) bool {
	ret := _mock.Called(hash, password)

	if len(ret) == 0 {
		panic("no return value specified for DoesMatch")
	}
...
// MockPasswordHasher_DoesMatch_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'DoesMatch'
type MockPasswordHasher_DoesMatch_Call struct {
	*mock.Call
}

// DoesMatch is a helper method to define mock.On call
//   - hash string
//   - password string
func (_e *MockPasswordHasher_Expecter) DoesMatch(hash interface{}, password interface{}) *MockPasswordHasher_DoesMatch_Call {
	return &MockPasswordHasher_DoesMatch_Call{Call: _e.mock.On("DoesMatch", hash, password)}
}

func (_c *MockPasswordHasher_DoesMatch_Call) Run(run func(hash string, password string)) *MockPasswordHasher_DoesMatch_Call {
...
func (_c *MockPasswordHasher_DoesMatch_Call) Return(b bool) *MockPasswordHasher_DoesMatch_Call {
	_c.Call.Return(b)
	return _c
}

func (_c *MockPasswordHasher_DoesMatch_Call) RunAndReturn(run func(hash string, password string) bool) *MockPasswordHasher_DoesMatch_Call {
	_c.Call.Return(run)
	return _c
}
